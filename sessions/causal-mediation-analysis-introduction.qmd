...
When we want to understand not just **if**, but **how** red meat
consumption affects blood glucose, we need to consider mediating
factors. For example:

Will eating more red meat increase my body fat percentage, which in turn
increases my blood glucose levels?

This requires us to decompose the total effect into direct and indirect
pathways. For natural mediation effects, we need to consider scenarios
that cross between different possible worlds, such as:

What would my blood glucose level be if I ate more red meat (treatment =
1), BUT my body fat percentage remained at the level it would have been
had I NOT eaten more red meat (mediator at level from treatment = 0)?
...
```{r} # This includes readr, or use library(readr) specifically nhanes <- read_csv(here::here("data/NHANES.csv"))  # keep the variables used in the analyses  data <- nhanes %>%    dplyr::select(ID,                  w1 = Age,                  w2 = Gender,                  w3 = Education,                  w4 = Smoke100,                  a = PhysActive,  # this is the exposure                 m = BMI,         # this is the mediator                 y = Pulse,       # this is the outcome                 y2 = Diabetes ) %>%    na.omit()}
```
...
```{r} lm_m <- lm(m ~ a + w1 , data = data) summary(lm_m)}
```
...
```{r} lm_y <- lm(y ~ a + m + a:m + w1 , data = data) summary(lm_y)}
```
...
```{r} data %>%   select(m) %>%   summary()  median_m <- round(median(data$m), digits = 1)}
```
...
```{r} CDE_m <- (lm_y$coefficients[2] + lm_y$coefficients[5] * median_m) * (1 - 0) round(as.numeric(CDE_m), digits = 2)}
```
...
```{r} mean_w1 <- mean(data$w1) mean_w1}
```

Then we calculate the PNDE.

```{r} PNDE <- (lm_y$coefficients[2] + (lm_y$coefficients[5] * (lm_m$coefficients[1] + lm_m$coefficients[2] * 0 + lm_m$coefficients[3] * mean_w1))) * (1 - 0) round(as.numeric(PNDE), digits = 2)}
```
...
```{r} TNDE <- (lm_y$coefficients[2] + (lm_y$coefficients[5] * (lm_m$coefficients[1] + lm_m$coefficients[2] * 1 + lm_m$coefficients[3] * mean_w1))) * (1 - 0)  round(as.numeric(TNDE), digits = 2)}
```
...
```{r} TNIE <- ((lm_y$coefficients[3] * lm_m$coefficients[2]) + (lm_y$coefficients[5] * lm_m$coefficients[2] * 1)) * (1 - 0)  round(as.numeric(TNIE), digits = 2)}
```
...
```{r} PNIE <- ((lm_y$coefficients[3] * lm_m$coefficients[2]) + (lm_y$coefficients[5] * lm_m$coefficients[2] * 0)) * (1 - 0)  round(as.numeric(PNIE), digits = 2)}
```
...
```{r} TE <- PNDE + TNIE  round(as.numeric(TE), digits = 2)}
```
...
```{r} PM <- TNIE / TE  as.numeric(PM) * 100}
```
