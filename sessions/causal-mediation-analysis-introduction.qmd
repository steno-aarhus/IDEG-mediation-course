...
-   BMI (m) is associated with higher pulse, independent of physical
    activity
...
average if the mediator, here BMI, were fixed at level (m =
`r median_m`) uniformly in the population but the treatment, physical
activity, was changed from 0 to 1.
...
treatment a, physical activity, was set at 1 versus 0 but for each
...
level it would take if a\* = 0 to the level it would take if a =1
(physical active).
...
cause higher blood glucose via inflammation only (i.e., due to red meat
affecting inflammation and subsequently, inflammation affecting blood
glucose), not accounting for the possible interaction between physical
activity and BMI? In other words, the effect that the exposure would
have had if 'its only action were to cause the mediator' (i.e., the
portion of the effect for which mediation is ''sufficient'')"
@wang_g-computation_2015.
