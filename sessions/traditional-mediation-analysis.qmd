...
It has been suggested that physical activity is associated with a reduced risk of Type 2 diabetes (T2D). The predominant hypothesis is that physical activity contributes to weight reduction, which in turn improves lipid metabolism and lowers the risk of developing T2D. Therefore, we aim to investigate the extent to which the effect of physical activity on lipid metabolism (e.g., total cholesterol) is mediated by BMI.

The situation is depicted by below figure.

> Does physical activity lead to reduce levels of total cholesterol by mediating BMI?
...
We'll explore the relationship between physical activity and cholesterol in the National Health and Nutrition Examination Survey (NHANES) dataset, which has been modified for this course with simulated variables.
...
-   exposure of interest (a): physical activity (continuous variable, grams per day)

-   outcome (y): cholesterol (continuous variable)
...
Here, we see that physical activity is negatively associated with BMI (an increase of 1 unit in physical activity is associated with a 1.76 unit increase in BMI).

### Step 2: Estimate the relationship between M on Y controlling for A (path b)

That is to build the model between BMI and chlesterol levels, controlling for physical activity. This step corresponds to criteria (ii).
...
After adjusting for physical activity, BMI is associated with lower cholesterol (an increase of 1 unit in BMI is associated with 0.02 unit reduction in the blood cholesterol).

### Step 3: Estimate the relationship between A on Y (path c)

That is to build the model between physical activity and cholesterol levels, without adjusting for the BMI. The only difference between step 2 and step 3 is that mediator is not included in the model.
...
We can see that red meat intake is strongly associated with higher glucose levels (an increase of 1 unit in red meat consumption is associated with a 70.5 unit increase in the blood glucose.
...
The *direct effect* ($\theta_1$) of physical activity on cholesterol levels can be assessed by the coefficient of physical activity in model b.
...
The *indirect effect* can be assessed by calculating the product of $\beta_1$ $\theta_2$. In this example, the indirect effect between physical activity and cholesterol levels through BMI is 0.036.
...
The *total effect* ($\theta_1'$) of physical activity on cholesterol levels can be assessed by the coefficient of physical activity in model c, which is 0.07. You might have noticed the total effect also equals the sum of the direct effect and indirect effect.
...
Back to the example, the indirect effect will be the coefficient of physical activity in model c (without adjusting for m) minus the coefficient of physical activity in model b (adjusting for m), which equals to 0.036.
...
-   **ACME** stands for average causal mediation effects. This is the indirect effect of the IV on the DV that goes through the mediator. Note that ACME estimated to be 0.0365, which is the same to the traditional approached we just used.
...
-   **Prop. Mediated** describes the proportion of the effect of the IV on the DV that goes through the mediator. It's calculated by dividing the ACME (0.0365) by the total effect (0.072) and yields 0.506. The result suggests the mediator (BMI) explains 50.6% proportion of the effect of physical activity on cholesteol levels. We will talk more about proportion mediation in the next session.
